...
List接口下应该有一个contains方法。底层的逻辑是遍历所有的元素。这个非常恐怖！严重会导致CPU打满，性能消耗极大。
所以判断存在否，建议使用HashSet。
...
写的时候就是先拷贝当前的一份到另一个新的内存空间，然后对这个新的数组进行写的操作。读的操作依旧是在原来的内存上，而且不加锁。


#### BlobkingQueue

阻塞队列，就是参与的三方：生产者，消费者和队列本身的交互。

当队列满的时候，生产者是阻塞住的，等待队列有空间可以继续塞进去。
当队列空的时候，消费者是阻塞住的，等待队列有东西的时候继续消费。

> 理解这里的阻塞非常重要，这个也是协作的重点。
